#!/usr/bin/env bash
# scripts/pages/resolve-django-python-quality-source.sh
#
# Resolves the Django Python Quality workflow run whose Qodana report
...
WORKFLOW_NAME="Django Python Quality"
...
if ! curl -fsSL \
  ...
  "https://api.github.com/repos/${GITHUB_REPOSITORY}/actions/workflows/django_python_quality.yml/runs?branch=main&status=completed&per_page=20" \
  -o "$response_file"; then
  echo "Warning: GitHub API request for Django Python Quality workflow runs failed; Pages will publish without a hosted Django Python quality report."
  exit 0
fi
...
if [[ -n "$resolved" ]]; then
  ...
  echo "No successful Django Python Quality workflow run on main was found; Pages will publish without a hosted Django Python quality report."
fi