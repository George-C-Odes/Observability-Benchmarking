...
## Overview
A high-performance REST service implementation built with Helidon 4.5.5 MicroProfile running on the Java Virtual Machine (JVM 25). Helidon 4 MP uses CDI (Weld) for dependency injection, JAX-RS (Jersey) for routing, and virtual threads by default for every request handler.
...
### Framework & Runtime
- **Framework**: Helidon 4.5.5 MP (CDI + JAX-RS)
...
**Approach**: Helidon MP Telemetry + OTel SDK autoconfigure + OTLP/gRPC exporter
...
All three signal pipelines share the same SDK instance, configured via `OTEL_*` environment variables in Docker Compose. The Compose defaults use `OTEL_EXPORTER_OTLP_ENDPOINT=http://alloy:4317`, `OTEL_EXPORTER_OTLP_PROTOCOL=grpc`, and OTLP exporters for traces, metrics, and logs. Alloy receives these over gRPC and batches them before forwarding to Grafana LGTM components.

Helidon's `helidon-microprofile-telemetry` dependency owns OTel SDK initialization and request tracing. The remaining application wiring is limited to benchmark-local Micrometer meters and Logback logs.
...
**Image**: `helidon-mp-jvm:4.5.5_latest`
...
4. **Helidon MP Telemetry**: `helidon-microprofile-telemetry` provides the CDI-managed OTel SDK and JAX-RS trace filters, replacing custom early-bootstrap tracing code.
