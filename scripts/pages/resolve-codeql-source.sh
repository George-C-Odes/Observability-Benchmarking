#!/usr/bin/env bash
# scripts/pages/resolve-codeql-source.sh
#
# Resolves the CodeQL workflow run whose quality report artifact
# should be published to GitHub Pages.
#
# Required env:
...
# CodeQL uploads its report artifact even when security issues are found
...
if ! curl -fsSL \
  ...
  "https://api.github.com/repos/${GITHUB_REPOSITORY}/actions/workflows/codeql.yml/runs?branch=main&status=completed&per_page=20" \
  -o "$response_file"; then
  echo "Warning: GitHub API request for CodeQL workflow runs failed; Pages will publish without a hosted CodeQL quality report."
  exit 0
fi
...
if [[ -n "$resolved" ]]; then
  ...
  echo "No completed CodeQL workflow run with conclusion success or failure on main was found; Pages will publish without a hosted CodeQL quality report."
fi
