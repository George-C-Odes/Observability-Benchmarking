<!-- Generated from `services/java/spring/native/tomcat/README.template.md` via `scripts/render-readmes.mjs`. Do not edit `services/java/spring/native/tomcat/README.md` directly. -->

# Spring Boot Tomcat Native Service

## Overview
A REST service implementation built with Spring Boot {{SPRING_BOOT_VERSION}} compiled to a native executable using GraalVM Native Image. This service runs on embedded Tomcat and can be configured for either platform thread mode or virtual thread mode, offering fast startup times and low memory footprint.
...
### Framework & Runtime
- **Framework**: Spring Boot {{SPRING_BOOT_VERSION}} + Spring Web MVC
