...
## Overview
A high-performance REST service implementation built with Quarkus 3.35.4 running on the Java Virtual Machine (JVM 25). This service supports three different thread models in a single deployment, making it ideal for benchmarking different concurrency approaches.
...
**Image**: `quarkus-jvm:3.35.4_latest`
...
docker buildx build `
  ...
  -t quarkus-jvm:3.35.4_latest `
  --build-arg QUARKUS_VERSION=3.35.4 `
  --build-arg BUILDKIT_BUILD_NAME=quarkus-jvm:3.35.4_latest `
  --load `
  services/java
```
