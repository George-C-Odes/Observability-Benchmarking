...
## Overview
A REST service implementation built with Spring Boot 4.1.1 running on Apache Tomcat embedded server with the Java Virtual Machine (JVM 25). This service can be configured to run in either platform thread mode or virtual thread mode, but only one mode per deployment.
