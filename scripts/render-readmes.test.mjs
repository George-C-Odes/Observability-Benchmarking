...
function testRenderTemplate() {
  ...
  assert.equal(rendered, 'Spring 4.1.0 / Go 1.26.6');

  const githubActionsSnippet = renderTemplate('Build ${{ matrix.service.name }} with {{GO_VERSION}}', {
    GO_VERSION: '1.26.6',
  });

  assert.equal(githubActionsSnippet, 'Build ${{ matrix.service.name }} with 1.26.6');

  const jekyllSnippet = renderTemplate("Image {{ '/images/foo.png' | relative_url }} with {{GO_VERSION}}", {
    GO_VERSION: '1.26.6',
  });

  assert.equal(jekyllSnippet, "Image {{ '/images/foo.png' | relative_url }} with 1.26.6");
...
function testRenderTemplatesResolvesRelativeEnvPath() {
  ...
  try {
    ...
    assert.equal(readFileSync(outputPath, 'utf8'), 'Spring 4.1.0 / Quarkus 3.38.2');
  } finally {
    try {
...
function testRealignMarkdownTables() {
  ...
  const ragged = [
    ...
    '| Go     | 1.26.6          | Alt runtime    |',
    '| Node   | 26.7.0                  | Frontend       |',
  ].join('\n');
