...
## Overview
A high-performance REST service implementation built with Quarkus 3.32.2 running on the Java Virtual Machine (JVM 25). This service supports three different thread models in a single deployment, making it ideal for benchmarking different concurrency approaches.
...
docker buildx build `
  ...
  -t quarkus-jvm:3.32.2_latest `
  --build-arg QUARKUS_VERSION=3.32.2 `
  --build-arg BUILDKIT_BUILD_NAME=quarkus-jvm:3.32.2_latest `
  --load `
  services/java
```
