 ...
 *
 * <ul>
 *   <li>{@code log} – stream output line (stdout / stderr)
 *   <li>{@code status} – human-friendly status message for display / debugging
 *   <li>{@code summary} – machine-readable snapshot (queued / running / terminal)
 *   <li>{@code terminalSummary} – machine-readable terminal snapshot
 * </ul>
 *
...
public record JobEvent(
  ...
  /**
   * Maps an application-layer job event to the API representation.
   *
   * @param event application-layer job event
   * @return API job event
   */
  public static JobEvent from(
      io.github.georgecodes.benchmarking.orchestrator.application.job.JobEvent event) {
    return new JobEvent(
        event.type(),
        event.stream(),
        event.ts(),
   ...
   */
  private static JobEvent buildSnapshot(
      String type,
      UUID jobId,
      String jobStatus,
      Instant createdAt,
