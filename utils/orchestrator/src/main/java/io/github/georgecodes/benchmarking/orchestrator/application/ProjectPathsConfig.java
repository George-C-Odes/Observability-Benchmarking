 ...
 * as an {@link Optional} return type (the intended Java idiom) rather than an {@code Optional}
 * field.
 */
...
public interface ProjectPathsConfig {
  ...
  /**
   * Host-side project directory (used when the orchestrator runs docker compose against a host
   * Docker Engine). When present, this path is treated as trusted (and does not need to be under
   * the workspace root) to validate compose-related paths. It is not automatically injected as
