...
## Overview
A REST service implementation built with Spring Boot 4.0.0 compiled to a native executable using GraalVM Native Image. This service runs on embedded Tomcat and can be configured for either platform thread mode or virtual thread mode, offering fast startup times and low memory footprint.
...
### Framework & Runtime
- **Framework**: Spring Boot 4.0.0 + Spring Web MVC
...
```promql
# RPS
rate(spring_request_count_total{service_name="SpringNativeTomcat"}[1m])
