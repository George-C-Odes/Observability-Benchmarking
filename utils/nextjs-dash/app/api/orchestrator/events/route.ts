  ...
  async function GET(request: NextRequest, ctx: ApiRouteContext) {
    ...
    try {
      ...
      if (runId && activeRunId && runId !== activeRunId) {
        ...
        return errorJson(
          409,
          { error: 'stale_run', message: 'This SSE stream request is for a stale run.' },
          requestId ? { headers: { 'X-Request-Id': requestId } } : undefined,
        );
      }

      serverLogger.debug('SSE request received', {
        jobId,
        runId: runId ?? null,
      ...
      if (!response.ok) {
        ...
        serverLogger.warn('Orchestrator SSE returned non-OK', {
