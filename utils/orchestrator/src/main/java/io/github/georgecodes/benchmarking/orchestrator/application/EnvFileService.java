...
/**
 * Service for managing environment configuration files. Handles file I/O, validation, and backup
...
public class EnvFileService {
  ...
  /**
   * Creates an environment file service.
   *
   * @param paths strongly typed project-path configuration
   */
  @Inject
  public EnvFileService(ProjectPathsConfig paths) {
    this.paths = paths;
  }
  ...
  public EnvFileContent readEnvFile() {
    ...
    try {
      ...
      throw new EnvFileException(
          "Failed to read environment file: " + e.getMessage(), Type.IO_ERROR, e);
    }
  }

  /**
   * Updates the environment file with new content. Automatically creates a timestamped backup
   * before modification.
   *
  ...
  public EnvFileUpdate updateEnvFile(String newContent) {
    ...
    try {
      ...
      throw new EnvFileException(
          "Failed to update environment file: " + e.getMessage(), Type.IO_ERROR, e);
    }
  }
   ...
   */
  public record EnvFileContent(String content, @JsonProperty("path") String absolutePath) {}
   ...
   */
  public record EnvFileUpdate(String message, @JsonProperty("backup") String backupFilename) {}
