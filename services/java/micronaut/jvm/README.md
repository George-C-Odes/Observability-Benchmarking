...
## Overview
A high-performance REST service implementation built with Micronaut 5.1.3 running on the Java Virtual Machine (JDK 25). This service supports three concurrency models — platform threads, virtual threads, and reactive (Reactor) — in a single deployment via endpoint routing, making it ideal for benchmarking different concurrency approaches with minimal configuration overhead.
...
**Image**: `micronaut-jvm:5.1.3_latest`
...
docker buildx build `
  ...
  -t micronaut-jvm:5.1.3_latest `
  --build-arg MICRONAUT_VERSION=5.1.3 `
  --build-arg BUILDKIT_BUILD_NAME=micronaut-jvm:5.1.3_latest `
  --load `
  services/java
```
