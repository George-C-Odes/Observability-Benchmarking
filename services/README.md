...
docker buildx build `
  ...
  -t quarkus-jvm:3.38.3_latest `
  --build-arg QUARKUS_VERSION=3.38.3 `
  --build-arg BUILDKIT_BUILD_NAME=quarkus-jvm:3.38.3_latest `
  --load `
  services/java
```
**quarkus-native**
...
docker buildx build `
    ...
    -t quarkus-native:3.38.3_latest `
    --build-arg QUARKUS_VERSION=3.38.3 `
    --build-arg BUILDKIT_BUILD_NAME=quarkus-native:3.38.3_latest `
    --load `
    services/java
```
**micronaut-jvm**
...
docker buildx build `
  ...
  -t helidon-se-jvm:4.5.3_latest `
  --build-arg HELIDON_VERSION=4.5.3 `
  --build-arg BUILDKIT_BUILD_NAME=helidon-se-jvm:4.5.3_latest `
  --load `
  services/java
```
**helidon-se-native**
