...
## Overview
A reactive REST service implementation built with Spring Boot 4.1.1 running on Netty web server (via Spring WebFlux) with the Java Virtual Machine (JVM 25). This service uses reactive programming with Project Reactor for non-blocking, event-driven request handling.
